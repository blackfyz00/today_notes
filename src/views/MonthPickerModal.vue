...
<template>
  <!-- Рендерим контент только если менеджер передал флаг show -->
  <div v-if="show" class="month-picker-overlay" @click="closeModal">
...
// Контракт пропсов, соответствующий нашему ModalManager
const props = defineProps<{
  show: boolean; // Обязательный флаг видимости в стеке
  currentDate: Date;
}>()
...
const isActive = (monthIndex: number) => {
  return (
...
const closeModal = () => {
  emit('close') // Передаем событие закрытия наверх менеджеру модалок
}
